...
fitcumordinal <- clm(Cat ~   age+sex+trestbps+chol+fbs+restecg+thalach+exang+oldpeak +chest_pain,  data=dat, doFit = TRUE, model = TRUE,  link = "logit")
summary(fitcumordinal)

#dat$pain4 <- relevel(dat$chest_pain, ref = 4)
#fitcumordinal <- clm(Cat ~   age+sex+trestbps+chol+fbs+restecg+thalach+exang+oldpeak +pain4,  data=dat, doFit = TRUE, model = TRUE,  link = "logit")
#summary(fitcumordinal)
...
### pain binary
bin<<- ifelse(dat$chest_pain == 4, 0, 1)
...
plot(adjshift,names=c("age","sex", "chest_pain"),colorvec=rep(1,5),KI = TRUE, KIfactor = 11/12,
     cex=1.4)
plot(adjshift,names=c("chol", "fbs"),colorvec=rep(1,5),KI = TRUE, KIfactor = 11/12,
     cex=1.4)
...
#lambdav<- c(0,1,5)
lambdav<- c(0,.3,1,3,6,10,16,30,50,100,300,500,1000) ## used!
#lambdav<- c(0,.5,1.5,2,3,5,8,10,15,20,30,50,100,300)
#lambdav<- c(0,2,3,5,8,10,15,20,30,50,150,300)  ##  

p<-dim(pred)[2]
pararr<-array(0,dim =c(length(lambdav),p,k-1)) ## lambda x variables x categories-1
#pararr[1,1,]
...
for (l in 1:length(lambdav)){
  if(l==1){
    fitl<-fitadjcats(resp,k,pred,deriv='der',hessian=FALSE,maxit=3000,start=0,lambda=lambdav[1],weightlasso,single=0)
  ...
  if(cross>0){
    ...
    for (ll in 1:crossnum){
      ...
      # fit
      if(l<2){
        fitc<-fitadjcats(respfit,k,predfit,deriv='der',hessian=FALSE,maxit=1000,start=fitl$parameter,lambda=lambdav[l],weightlasso,single=0)}
      if(l>1){
        fitc<-fitadjcats(respfit,k,predfit,deriv='der',hessian=FALSE,maxit=1000,start=fitl$parameter,lambda=lambdav[l],weightlasso,single=0)
        #fitc<-fitadjcats(respfit,k,predfit,deriv='der',hessian=FALSE,maxit=1000,
        #                 start=0,lambda=lambdav[l],weightlasso,single=0)
        }
      ...
      secfit<--loglikadjcatcats(fitc$parameter,respcrossnow,k,predcrossnow, 
                                lambda=0,weightlasso,single=rep(1:p))
      print(secfit)
...
for(v in 1:p){
  ...
  plot(log(1+lambda),pararr[,var,1], type="b", ylim=c(minpl,maxpl),ylab="",
       cex.lab=1.6,cex.axis=1.4,lwd=1.5, cex=1.2,pch = 1)
  title(ylab=(paste("variable",v)), line=2.32, cex.lab=1.7,lwd=2, cex=1.7)
  for( r in 2:(k-1))lines(log(1+lambda),pararr[,var,r], type="b",
                          cex.lab=1.6,cex.axis=1.4,lwd=1.5, cex=1.2,pch = 1)
}


#### plot cross
plot(log(1+lambdav),-loglikv,type='b',xlab="",ylab="",cex.lab=1.6,cex.axis=1.4,lwd=1.5, cex=1.2,pch = 1)
...
lamdnr<-c(1,3,5)
var<-1
plot(seq(2,k),pararr[lamdnr[1],var,],type='b',xlab="",ylab="",cex.lab=1.6,cex.axis=1.4,lwd=1.5, cex=1.2)
title(xlab=(paste("categories)")), cex.lab=1.7,lwd=2, cex=1.7)

for( l in 2:length(lamdnr)){
  lines(seq(2,k),pararr[lamdnr[l],var,],type='b',xlab="",ylab="",cex.lab=1.6,cex.axis=1.4,lwd=1.5, cex=1.2)
}
